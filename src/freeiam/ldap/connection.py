...
class Connection:
    ...
    async def bind(self, authc_id: str | None, password: str | None, *, controls: Controls | None = None) -> Result:
        """Authenticate via plaintext credentials."""
        conn = self.conn
        self._last_auth_state = ('simple_bind_s', authc_id, password)
        response = await self._execute(conn, conn.simple_bind, authc_id, password, **Controls.expand(controls))
        return Result.from_response(None, None, controls, response)

    async def bind_external(self, authz_id: str = '', *, controls: Controls | None = None) -> None:  # pragma: no cover
        """Authenticate via EXTERNAL method e.g. UNIX socket or TLS client certificate."""
        return await self.bind_sasl(ldap.sasl.external(authz_id), controls=controls)

    async def bind_gssapi(self, authz_id: str = '', *, controls: Controls | None = None) -> None:  # pragma: no cover
        """Authenticate via GSSAPI e.g. via Kerberos ticket."""
        return await self.bind_sasl(ldap.sasl.gssapi(authz_id), controls=controls)

    async def bind_oauthbearer(
        self, token: str, authz_id: str = '', *, controls: Controls | None = None
    ) -> None:  # pragma: no cover; unavailable lib
        """Authenticate via OAuth 2.0 Access Token."""
        return await self.bind_sasl(
            ldap.sasl.sasl(
                {
    ...
    async def bind_sasl(self, auth: ldap.sasl.sasl, *, controls: Controls | None = None) -> None:  # pragma: no cover
        """Authenticate via SASL."""
        with errors.LdapError.wrap(self._hide_parent_exception):
            self.conn.sasl_interactive_bind_s('', auth, **Controls.expand(controls), sasl_flags=ldap.SASL_QUIET)

    async def bind_sasl_noninteractive(self, sasl_mech: str, authz_id: str = '', *, controls: Controls | None = None) -> None:  # pragma: no cover
