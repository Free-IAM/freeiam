...
def test_escape():
    ...
    assert (
        Filter.escape(ascii_chars, EscapeMode.NON_ASCII)
    ...
    assert (
        Filter.escape(latin_chars, EscapeMode.NON_ASCII)
...
def test_example_walk_replace():
    disabled = Filter('(&(shadowExpire=1)(krb5KDCFlags:1.2.840.113556.1.4.803:=128)(|(sambaAcctFlags=[UD       ])(sambaAcctFlags=[ULD       ])))').ast
    enabled = Filter(
        '(&(!(shadowExpire=1))(!(krb5KDCFlags:1.2.840.113556.1.4.803:=128))(!(|(sambaAcctFlags=[UD       ])(sambaAcctFlags=[ULD       ]))))'
    ).ast

    def comparision_callback(filter_, parent, expression):
        if expression.attr == 'disabled':
            ...
            parent.replace(expression, Filter.from_format('(&(cn=%s)(associatedDomain=%s))', [host, domain]).ast)
        elif expression.attr == 'alias':
            expression.attr = 'resolved'
            if expression.value == 'no':
            ...
            aaa = expression.copy()
            a.attr = 'aAAARecord'
            parent.replace(expression, OR([a, aaa]))
        elif expression.attr == 'birthdate':
            from datetime import datetime

            dt = datetime.strptime(expression.value, '%Y-%m-%d')  # noqa: DTZ007
            parent.replace(expression, ExtensibleMatch('dateOfBirth', dt.strftime('%Y%m%d%H%M%SZ'), '', 'generalizedTimeMatch'))
        else:
            return
    ...
    def operator_callback(filter_, parent, expression):
        if isinstance(expression, AND):
            parent.replace(expression, OR(expression.expressions))

    fil = Filter(
        '(&(disabled=1)(fqdn=ldap.freeiam.org)(alias=no)(unlocked=1)(ip=127.0.0.1)(ip=0000:0000:0000:0000:0000:0000:0000:0001)(a=192.168.0.1)(birthdate=2025-07-04))'
    )
    fil.walk(comparision_callback, operator_callback)
    result = r"""
(|
  (&
    (shadowExpire=1)
  ...
  (dateOfBirth:generalizedTimeMatch:=20250704000000Z)
    ...
    assert (
        str(fil)
        == '(|(&(shadowExpire=1)(krb5KDCFlags:1.2.840.113556.1.4.803:=128)(|(sambaAcctFlags=[UD       ])(sambaAcctFlags=[ULD       ])))(&(cn=ldap)(associatedDomain=freeiam.org))(resolved=yes)(!(locked=1))(aRecord=127.0.0.1)(aAAARecord=0000:0000:0000:0000:0000:0000:0000:0001)(|(aAAARecord=192.168.0.1)(a=192.168.0.1))(dateOfBirth:generalizedTimeMatch:=20250704000000Z))'  # noqa: E501
    )
